@echo off
    ...
    for /l %%f in (0 1 3)do (
	Timeout 1 >NUL
        call :drawProgressBar 20 "Cheems OS Arch [Making Main File]"
	Timeout 1 >NUL
        call :drawProgressBar 30 "Cheems OS Arch [Making Folders]"
	Timeout 1 >NUL
        call :drawProgressBar 90 "Cheems OS Arch [Xcopy INS]"
	Timeout 2 >NUL
        call :drawProgressBar 100 "Cheems OS Arch [Done]"
	pause >nul
	exit
    )
